...
1. `cg-graphify-bridge faq-prep . --out graphify-out` — writes one task per stale/missing feature
   (graph facts + the existing narrative) plus a project-level task under
   `graphify-out/.cache/faq/tasks/`.
2. For each task, write the narrative payload it asks for under `graphify-out/.cache/faq/payloads/`
   — short, factual prose grounded in the listed members/inputs/outputs; never invent anchors.
3. `cg-graphify-bridge faq-merge . --out graphify-out` — merges into `graphify-out/faq.json` and
   stamps the per-feature baseline. Then `git add graphify-out/faq.json` and commit. CI renders
